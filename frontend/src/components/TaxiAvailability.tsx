...
export const TaxiAvailability = ({ route, onBookSeat }: TaxiAvailabilityProps) => {
  const [taxis] = useState<Taxi[]>(mockTaxis);
  ...
  const handleNotifySubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      toast({
        title: "Alert saved",
        description: `We will message ${notifyName} at ${notifyPhone} as soon as a seat opens.`,
      });
  ...
  const hasAvailableSeats = taxis.some((taxi) => taxi.seatsAvailable > 0);
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {taxi.seatsAvailable}/{taxi.totalSeats} seats
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {Array.from({ length: taxi.totalSeats }).map((_, i) => (
                    <div
                      ...
                      className={`w-6 h-6 rounded border-2 ${
                        i < taxi.totalSeats - taxi.seatsAvailable
                          ? "bg-muted border-muted-foreground"
                          : "bg-success border-success"
                      }`}
                      title={
                        i < taxi.totalSeats - taxi.seatsAvailable ? "Occupied" : "Available"
                      }
                    />
                  ))}
                </div>
        ...
        <Card className="text-center p-8">
          <CardContent className="space-y-4">
            ...
            <p className="text-muted-foreground">
              Check back in a few minutes or try a different route.
            </p>
            <Dialog open={notifyOpen} onOpenChange={handleDialogChange}>
              ...
              <DialogContent>
                <form onSubmit={handleNotifySubmit} className="space-y-4">
                  <DialogHeader>
                    <DialogTitle>Get seat alerts</DialogTitle>
                    <DialogDescription>
                      Share your details and we will send you a message as soon as seats open up.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <DialogFooter className="sm:justify-between">
                    ...
                    >
                      {notifySubmitting ? "Saving..." : "Save Alert"}
                    </Button>
                  </DialogFooter>
                </form>
              </DialogContent>
