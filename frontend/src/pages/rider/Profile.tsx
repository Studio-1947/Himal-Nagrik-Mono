...
type RiderSettingsValues = z.infer<typeof riderSettingsSchema>;

const RiderProfilePage = () => {
  const { profile, updateProfile, isPending, logout } = useAuth();
  const navigate = useNavigate();
  const formValues = useMemo<RiderSettingsValues>(() => {
  ...
  const form = useForm<RiderSettingsValues>({
  ...
  const handleSubmit = async (values: RiderSettingsValues) => {
    try {
      const favouriteRoutes = values.favouriteRoutes
        ?.split(",")
      ...
      await updateProfile({
        ...
        emergencyContact:
          values.emergencyContactName || values.emergencyContactPhone || values.emergencyContactRelation
            ? {
      ...
      toast({
        title: "Profile saved",
        description: "Your rider preferences were updated successfully.",
      });
    } catch (error) {
  ...
  const completedTrips = profile.recentTrips.filter((trip) => trip.status === "completed").length;
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge className="bg-emerald-500/15 text-emerald-200">Rider</Badge>
              </div>
              <p className="text-sm text-slate-300">{profile.bio || "Add a short bio to let drivers know you."}</p>
        ...
        <main className="mt-10 grid gap-10 lg:grid-cols-[2fr,1fr]">
          <section className="space-y-8">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl">
              <h2 className="text-lg font-semibold text-white">Update your preferences</h2>
              <p className="text-sm text-slate-300">
                Tailor your shared rides and keep emergency contacts handy for faster dispatch.
              </p>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Darjeeling ↔ Siliguri, Darjeeling ↔ Mirik" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="front / middle / back" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="quiet / neutral / lively" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
          ...
          <aside className="space-y-6">
                ...
                {profile.recentTrips.map((trip) => (
                  <div key={trip.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="flex items-center justify-between text-sm text-slate-200">
                      ...
                      <Badge className="bg-emerald-500/15 capitalize text-emerald-200">{trip.status}</Badge>
                    </div>
                    <p className="mt-2 text-sm font-medium text-white">{trip.route}</p>
            ...
            <div className="rounded-3xl border border-white/10 bg-emerald-500/10 p-6 backdrop-blur-xl text-sm text-emerald-100">
              ...
              <p className="mt-3 text-emerald-100/90">
                Leaving early from Chowk Bazaar? Mark the route as favourite to get automatic reminders when seats fill up.
              </p>
            </div>
          </aside>
        </main>
